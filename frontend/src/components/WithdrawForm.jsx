...
import { retrait } from "../api";
import styles from "./OpForm.module.css";

export default function WithdrawForm({ id, onSuccess }) {
  ...
  async function handleSubmit(e) {
    ...
    setMsg(null);
    try {
      const data = await retrait(id, val);
      if (data.succes) {
  ...
  return (
    <div className={`card ${styles.card}`}>
      <h3 className={styles.title}>💸 Retrait</h3>
      <form onSubmit={handleSubmit} className={styles.form}>
        ...
        <button type="submit" className={`btn-warning ${styles.btn}`} disabled={loading}>
